...
width = 3; step=1; start=4;

% train test diagnostics
testaccuracy = zeros(30, n_subj);
trainaccuracy = zeros(30, n_subj);
testaccuracy_perm = zeros(30, n_subj, 5);

for W = start:step:30    
optload.freq = [W W+width; W-2 W+width+2; W-1 W+width+1]; %frequency bands window for SSD
...
%save results for each Sliding window
testaccuracy(W, 1:s) = AUC_cv1';
trainaccuracy(W, 1:s) = AUC_cv1_tr';
%testaccuracy_perm(W, 1:s, :) = permute(AUC_cv1_p, [3 1 2]);
empirical_pval{W,1} = p; %perm_pval{W,1} = p_p;
test_stat{W,1} =  Stat; %test_perm_stat{W,1} =  Stat_p; 
end

% Plot subject statistics
testaccuracy(:,13) = []; testaccuracy(1:start-1,:) = [];
trainaccuracy(:,13) = []; trainaccuracy(1:start-1,:) = [];
testaccuracy_perm(:,13,:) = []; testaccuracy_perm(1:start-1,:,:) = [];
...
subplot(3,2,pos(s))
hold on
plot(start:step:30, testaccuracy(:,s))
plot(start:step:30, trainaccuracy(:,s))
plot(0:2:30, repmat(50,1,16), 'color', 'k', 'LineStyle', ':')
xlim([start 30])
ylim([46 80])
xticks(start:2:30)
yticks(48:2:80)
title(['\bf\fontsize{8} Classification of intensity rating per window index for subject #' num2str(s)])
xlabel('Window index');
ylabel('Acc');
...
xticks(6:2:34)
yticks(48:2:70)
title({'{\bf\fontsize{14} Classification of perceived intensity depending on SSD signal band}'});
xlh = xlabel('Window index w');
ylh = ylabel('Mean performance over participants');
...
observed_data = testaccuracy(7,:)';
permtest_data = permute(testaccuracy_perm(7,:,:), [3 1 2]);
