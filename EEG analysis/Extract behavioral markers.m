...
for s = subject_vector
   ...
   % sampling points to ms
    ...
    %leave out "ix_accepted" to make this code work with this version. no epoching like previously in "Behavioral data.m", the rejected epochs will be excluded in the CSP script
    %zero_idx = task_perf == 0; %remove trials without response (task_perf == 0)
    %ix_accepted = ix_accepted.*abs(zero_idx-1)'; ix_accepted (ix_accepted == 0) = []; 
    
    
    %subject vector
    subject = ones(1,length(ix_AB)) * subject_vector(s);
    ...
    behavior = [subject; task_perf; accuracies; judgement; latency]';
