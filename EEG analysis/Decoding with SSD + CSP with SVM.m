...
optload.screeplot = 0; %display screeplot of sorted eigenvalues 
...
[AUC_cv1(s,:), AUC_cv1_tr(s,:), p(s,1), stat, ] = crossvalidation_AK(fv, cvopt, {@fitcsvm, 'KernelFunction', 'rbf',...
                'KernelScale','auto', 'Standardize',true, 'OptimizeHyperparameters', 'none', 'Verbose', 0}, ...
                'LossFcn', {@loss_0_1 @loss_sensitivity @loss_specificity},... 
...
% Classification with data from first condition
[AUC_cv1_p(s,:,:), AUC_cv1_p_tr(s,:,:), p_p(s,1), stat_p] = crossvalidation_AK(fv, cvopt, {@fitcsvm, 'KernelFunction', 'rbf',...
                'KernelScale','auto', 'Standardize',true, 'OptimizeHyperparameters', 'none', 'Verbose', 0}, ...
                'LossFcn', {@loss_0_1 @loss_sensitivity @loss_specificity},... 
                'SampleFcn', {@sample_KFold, 10}, ... {@sample_leaveOneOut}
...
save('Decoding with SSD + CSP with SVM.mat')
...
permtest_data = permute(AUC_cv1_p(:,1,:), [2 1 3]);
...
ylim([45 70])
title({'{\bf\fontsize{14} SVM classification of intensity rating}'; '\itUsing SSD + CSP'});
